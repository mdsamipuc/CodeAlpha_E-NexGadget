                         ...
                         <div class="d-sm-flex justify-content-between mt-lg-4 mb-4 pb-3 pb-sm-2 border-bottom">
                            <div class="d-block d-sm-flex align-items-start text-center text-sm-start">
                                ...
                                <a class="d-block flex-shrink-0 mx-auto me-sm-4" href="{{route('front.product',$wishlist->product->slug)}}" style="width: 10rem;">
                                    ...
                                    <img src="{{asset('upload/product/small/'.$productImage->image)}}" class=""  >
                                    @else
                                    <img src="{{asset('admin_assets/img/default-150x150.png')}}" class="" >
                                    @endif
                                ...
                                <div class="pt-2">
                                    ...
                                    <span class="h5"><strong>{{$wishlist->product->price}} BDT</strong></span>
                                    @if ($wishlist->product->compare_price>0)
                                    <span class="h6 text-underline"><del>{{$wishlist->product->compare_price}} BDT</del></span>
                                    @endif
                                    </div>
                                </div>
                            </div>
...
<script>
    function removeProduct(id){
        $.ajax({
        url:'{{route("account.remove_product_from_wishlist")}}',
        type:'post',
        data:{id:id},
        dataType:'json',
